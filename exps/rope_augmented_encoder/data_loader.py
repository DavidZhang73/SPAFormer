...
class PartNetPartDataset(data.Dataset):
    def __init__(
        self, category, data_dir, data_fn, data_features, level, max_num_part=20, pose_sequence=1, rand_seq=0, pr=0
    ):
        # store parameters
    ...
    def __getitem__(self, index):
        ...
        if self.pose_sequence == 4:
            ...
            pose_sequence_fn = np.random.choice(
                ["pose_sequence_ascending_diagonal_only", "pose_sequence_box_asending", "pose_sequence"],
                p=[0.55, 0.35, 0.1],
            )
        ...
        if not self.pose_sequence == 0:
            try:
                cur_pose_sequence_fn = os.path.join(
                    self.data_dir, pose_sequence_fn + "/%s_level" % shape_id + "3" + ".txt"
                )
                cur_pose_sequence = np.array(np.genfromtxt(cur_pose_sequence_fn, delimiter=" ", dtype=np.int)).reshape(
                    -1
                )
                ...
                try:
                    cur_pose_sequence_fn = os.path.join(
                        self.data_dir, pose_sequence_fn + "/%s_level" % shape_id + "1" + ".txt"
                    )
                    cur_pose_sequence = np.array(
                        np.genfromtxt(cur_pose_sequence_fn, delimiter=" ", dtype=np.int)
                    ).reshape(-1)
                    cur_pose_sequence = cur_pose_sequence[:cur_num_part]
                    if self.pose_sequence == 7:
                        cur_pose_sequence = np.flip(cur_pose_sequence)
        ...
        for feat in self.data_features:
            if feat == "contact_points":
                ...
                cur_pts = self.file["part_pcs"][
                    shape_ind, :cur_num_part, :, :
                ]  # p x N x 3 (p is unknown number of parts for this shape)
                if not self.pose_sequence == 0:
