...
def train(conf):
    ...
    train_dataset = PartNetPartDataset(
        ...
        level=conf.level,
        pose_sequence=conf.pose_sequence,
        rand_seq=conf.rand_seq,
        pr=conf.pr,
    )
    ...
    for epoch in range(start_epoch, conf.epochs):
        ...
        for train_batch_ind, batch in train_batches:
            ...
            if conf.contact_loss:
                total_loss, total_trans_l2_loss, total_rot_cd_loss, total_shape_cd_loss, total_contact_loss = forward(
                    batch=batch,
                ...
                total_loss, total_trans_l2_loss, total_rot_cd_loss, total_shape_cd_loss = forward(
                    batch=batch,
                    data_features=data_features,
                    network=network,
                    conf=conf,
                        ...
                        if min_total_loss_val is None:
                            ...
                            if total_loss.item() < min_total_loss_val:
                                ...
                                utils.save_checkpoint(
                                    ...
                                    dirname=os.path.join(conf.exp_dir, "best"),
                                    epoch=epoch,
                                    prepend_epoch=False,
                                    optimizers=optimizers,
        ...
        # save checkpoint
        with torch.no_grad():
            if (last_checkpoint_step is None or epoch - last_checkpoint_step >= conf.checkpoint_interval) and int(
...
def forward(
    ...
    # prepare input
    input_part_pcs = batch[data_features.index("part_pcs")].cuda(
        non_blocking=True, device=torch.cuda.current_device()
    )  # B x P x N x 3
    input_part_valids = batch[data_features.index("part_valids")].cuda(
        non_blocking=True, device=torch.cuda.current_device()
    )  # B x P
    batch_size = input_part_pcs.shape[0]
    num_part = input_part_pcs.shape[1]
    num_point = input_part_pcs.shape[2]
    part_ids = batch[data_features.index("part_ids")].cuda(
        non_blocking=True, device=torch.cuda.current_device()
    )  # B x P
    ...
    gt_part_poses = batch[data_features.index("part_poses")].cuda(
        non_blocking=True, device=torch.cuda.current_device()
    )  # B x P x (3 + 4)
    position_index = (
        torch.tensor(range(0, num_part), dtype=torch.long)
        .repeat(input_part_pcs.shape[0], 1)
        .cuda(non_blocking=True, device=torch.cuda.current_device())
    )
    repeat_times = conf.repeat_times
    if conf.contact_loss:
        contact_points = batch[data_features.index("contact_points")].cuda(
            non_blocking=True, device=torch.cuda.current_device()
        )  # B x P x (3 + 4)
    ...
    with torch.cuda.amp.autocast(enabled=True):
        if not conf.MoN == 0:  # use MoN
            ...
            with torch.no_grad():
                for repeat_ind in range(repeat_times):
                    random_noise = np.random.normal(
                        loc=0.0, scale=1, size=[batch_size, num_parts, conf.random_noise_size]
                    ).astype(np.float32)  # B x P x 16
                    ...
                    for ind in range(len(batch[0])):
                        ...
                        for i in range(1, 10):
                            ...
                            if conf.num_gpus > 1:
                                matched_pred_ids, matched_gt_ids = network.module.linear_assignment_ddp(
                                    cur_input_pts, cur_pred_centers, cur_pred_quats, cur_gt_centers, cur_gt_quats
                                )
                            else:
                                matched_pred_ids, matched_gt_ids = network.linear_assignment_ddp(
                                    cur_input_pts, cur_pred_centers, cur_pred_quats, cur_gt_centers, cur_gt_quats
                                )
                    ...
                    if conf.num_gpus > 1:
                        trans_l2_loss_per_data = network.module.get_trans_l2_loss_ddp(
                            pred_part_poses[:, :, :3], gt_part_poses[:, :, :3], input_part_valids
                        )  # B
                        rot_cd_loss_per_data = network.module.get_rot_cd_loss_ddp(
                            input_part_pcs,
                            pred_part_poses[:, :, 3:],
                            gt_part_poses[:, :, 3:],
                            input_part_valids,
                        ...
                        shape_cd_loss_per_data = network.module.get_shape_cd_loss_ddp(
                            input_part_pcs,
                            pred_part_poses[:, :, 3:],
                            gt_part_poses[:, :, 3:],
                            input_part_valids,
                        ...
                        if conf.contact_loss:
                            contact_point_loss_per_data, _, _ = network.module.get_contact_point_loss_train(
                                pred_part_poses[:, :, :3],
                                pred_part_poses[:, :, 3:],
                                contact_points,
                                sym_info,
                        ...
                        trans_l2_loss_per_data = network.get_trans_l2_loss_ddp(
                            pred_part_poses[:, :, :3], gt_part_poses[:, :, :3], input_part_valids
                        )  # B
                        rot_cd_loss_per_data = network.get_rot_cd_loss_ddp(
                            input_part_pcs,
                            pred_part_poses[:, :, 3:],
                            gt_part_poses[:, :, 3:],
                            input_part_valids,
                        ...
                        if conf.contact_loss:
                            contact_point_loss_per_data, _, _ = network.get_contact_point_loss_train(
                                pred_part_poses[:, :, :3],
                                pred_part_poses[:, :, 3:],
                                contact_points,
                                sym_info,
            ...
            if conf.use_inst_encoded in [1]:
                pred_part_poses = network(
                    input_part_pcs, input_part_valids, None, position_index, part_ids, random_noise, instance_label
                )
            elif conf.use_inst_encoded in [2]:
                pred_part_poses_list = network(
                    input_part_pcs, input_part_valids, None, position_index, part_ids, random_noise, instance_label
                )
            ...
            if conf.num_gpus > 1:
                ...
                rot_cd_loss_per_data = network.module.get_rot_cd_loss_ddp(
                    input_part_pcs, pred_part_poses[:, :, 3:], gt_part_poses[:, :, 3:], input_part_valids, conf.device
                )  # B
                shape_cd_loss_per_data = network.module.get_shape_cd_loss_ddp(
                    input_part_pcs,
                    pred_part_poses[:, :, 3:],
            ...
            if conf.use_inst_encoded in [1]:
                pred_part_poses = network(
                    input_part_pcs, input_part_valids, None, position_index, part_ids, instance_label=instance_label
                )
            elif conf.use_inst_encoded in [2]:
                pred_part_poses_list = network(
                    input_part_pcs, input_part_valids, None, position_index, part_ids, instance_label=instance_label
    ...
    with torch.no_grad():
        ...
        if (
            ...
            if batch_ind < conf.num_batch_every_visu:
                ...
                pred_pts = qrot(
                    pred_part_poses[:, :, 3:].unsqueeze(2).repeat(1, 1, num_point, 1), input_part_pcs
                ) + pred_center.unsqueeze(2).repeat(1, 1, num_point, 1)
                gt_pts = qrot(
                    gt_part_poses[:, :, 3:].unsqueeze(2).repeat(1, 1, num_point, 1), input_part_pcs
                ...
                for i in range(batch_size):
                    ...
                    pred_part_pcs = qrot(
                        cur_pred_part_poses[:, 3:].unsqueeze(1).repeat(1, num_point, 1), cur_input_part_pcs
                    ) + cur_pred_part_poses[:, :3].unsqueeze(1).repeat(1, num_point, 1)
                    gt_part_pcs = qrot(
                        cur_gt_part_poses[:, 3:].unsqueeze(1).repeat(1, num_point, 1), cur_input_part_pcs
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--decoding_type", default="teacher_forcing", type=str, help="use our pose sequence or not, default is True"
    )
    parser.add_argument(
        "--schedule_sampling_prob", default=0, type=float, help="use our pose sequence or not, default is True"
    )
    parser.add_argument(
        "--pre_reasoning", default=False, type=bool, help="use our pose sequence or not, default is True"
    )
    ...
    parser.add_argument(
        "--random_noise_size", default=32, type=int, help="use our pose sequence or not, default is True"
    )
    parser.add_argument("--repeat_times", default=5, type=int, help="use our pose sequence or not, default is True")
    parser.add_argument(
        "--use_pretrained_models", default="no", type=str, help="use our pose sequence or not, default is True"
    )
    ...
    parser.add_argument(
        "--loss_weight_contact", default=20.0, type=float, help="use our pose sequence or not, default is True"
    )
