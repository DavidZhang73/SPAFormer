...
def test(conf):
    ...
    while val_batch_ind + 1 < val_num_batch:
        ...
        with torch.no_grad():
            ...
            scores = 0
            part_cd_loss, shape_cd_loss, contact_point_loss, acc_num, valid_num, max_count, total_num, success_rate = (
                forward(
                    batch=val_batch,
    ...
    utils.printout(
        conf.flog,
        f"total_success_rate:{total_success_rate}, total_num in validation: {total_number}, success_rate: {total_success_rate / total_number}",
    )
        ...
        for repeat_ind in range(repeat_times):
            if conf.MoN:
                random_noise = np.random.normal(
                    loc=0.0, scale=1, size=[batch_size, num_part, conf.random_noise_size]
            ...
            if conf.num_gpus > 1:
                ...
                rot_l2_loss_per_data = network.module.get_rot_l2_loss_ddp(
                    input_part_pcs, pred_part_poses[:, :, 3:], gt_part_poses[:, :, 3:], input_part_valids
                )  # B
                ...
                rot_l2_loss_per_data = network.get_rot_l2_loss_ddp(
                    input_part_pcs, pred_part_poses[:, :, 3:], gt_part_poses[:, :, 3:], input_part_valids
                )  # B
            ...
            # prepare gt
            input_part_pcs = input_part_pcs[:, :, :1000, :]
            total_cd_loss_per_data, acc = network.get_total_cd_loss(
                input_part_pcs,
                pred_part_poses[:, :, 3:],
                gt_part_poses[:, :, 3:],
                input_part_valids,
            ...
            contact_point_loss_per_data, count, total_num = network.get_contact_point_loss(
                pred_part_poses[:, :, :3], pred_part_poses[:, :, 3:], contact_points, sym_info
            )
        ...
        if is_val and (not conf.no_visu):
            ...
            if batch_ind < conf.num_batch_every_visu:
                ...
                for repeat_ind in range(repeat_times):
                    ...
                    pred_pts = qrot(
                        array_pred_part_poses[repeat_ind][:, :, 3:].unsqueeze(2).repeat(1, 1, num_point, 1),
                        input_part_pcs,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--use_pretrained_pct", default=0, type=int, help="use our pose sequence or not, default is True"
    )
    ...
    parser.add_argument(
        "--vis_number", default=10000000, type=int, help="use our pose sequence or not, default is True"
    )
